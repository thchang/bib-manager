...
if [ $OPTIND -eq 1 ]; then
  export CHECK_BIBMGR_SYNTAX=true;
...
if [ $CHECK_BIBMGR_SYNTAX == true ]; then
  echo
...
if [ $UNIT_TESTS == true ]; then
  ...
  echo
  python3 -m pytest -v --cov-config=bibmgr/tests/.coveragerc --cov=bibmgr --cov-report= bibmgr/tests/unit_tests -W error::UserWarning;
...
#  echo
#  for TEST_SCRIPT in bibmgr/tests/regression_tests/test_*.py; do
#    python3 -m coverage run --rcfile=bibmgr/tests/.coveragerc --append $TEST_SCRIPT
#
#    code=$? # capture pytest exit code
#    if [ "$code" -eq "0" ]; then
#      echo
